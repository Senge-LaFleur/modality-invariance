...
# ------------------------------------------------------------
# Single-Encoder Model (domain-shift ablation: BASE+SA+Contr, single-stream)
# ------------------------------------------------------------
class SingleStreamModel(nn.Module):
    """
    One backbone (resnet18 OR vit_small), one ProjectionHead, one
    classifier, one skin-type classifier. No 'clinical'/'derm' branching
    anywhere in forward() -- unlike DualResNet18/DualViT, there is no
    modality-based routing at all.

    WHY THIS EXISTS (don't skip this if you're reusing DualResNet18/DualViT
    "with one branch frozen" for a domain-shift experiment instead):
    DualResNet18/DualViT route each sample to clinical_backbone or
    derm_backbone based on batch["modality"], which reflects the sample's
    *source dataset*. If you train only on clinical data and then evaluate
    a derm_test loader through DualResNet18/DualViT, derm images get
    routed to derm_backbone -- a backbone that never trained on anything,
    since no gradients ever reached it. That measures "trained net vs
    untrained net," not domain shift. SingleStreamModel avoids this by
    construction: every image, clinical-sourced or derm-sourced, goes
    through the SAME encoder, so "same trained weights, different input
    distribution" is a valid measurement.

    forward(batch) always reads batch["clinical"]. This is safe (not a
    hack) because UnpairedDataset.__getitem__ always stores the *actual*
    loaded image under BOTH the 'clinical' and 'derm' keys, regardless of
    which modality the row came from. So batch["clinical"] is an
    unambiguous handle on "the image" whether the row originated from
    clin_*.csv, derm_*.csv, paired_clin_*.csv, or paired_derm_*.csv, as
    long as it was loaded through UnpairedDataset (which is all
    build_domain_shift_loaders uses -- PairedDataset rows, which need BOTH
    images at once, are intentionally never used for this experiment).
    """
    def __init__(self, backbone, embed_dim, num_classes, num_skin_types,
                 pretrained=True, use_projection=True):
        super().__init__()
        assert backbone in ("resnet18", "vit_small"), f"unknown backbone {backbone}"
        ...
        # Names ('classifier', 'skin_clf', 'proj_head') deliberately match
        # DualResNet18/DualViT so get_layer_wise_lr_params() below works
        # unmodified on this model too.
        self.classifier = nn.Sequential(nn.Dropout(0.3), nn.Linear(embed_dim, num_classes))
        self.skin_clf = nn.Sequential(
            nn.Linear(embed_dim, 256), nn.GELU(), nn.Dropout(0.2),
            nn.Linear(256, num_skin_types)
    ...
    def encode(self, x):
        f = self.encoder(x)
        z = self.proj_head(f) if self.use_projection else f
        return f, z
