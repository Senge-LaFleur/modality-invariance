...
Import these in training scripts.
"""
...
class DualResNet18(nn.Module):
    ...
    def forward(self, batch):
        ...
        embed_dim = self.classifier[1].in_features  # actual embedding dimension
        ...
        if paired_mask.any() and "clinical" in batch and "derm" in batch:
            clin_t = batch["clinical"].to(device)
            derm_t = batch["derm"].to(device)
        ...
        if unpaired_mask.any() and "image" in batch:
            ...
            modality = "clinical"
            for i, is_paired in enumerate(batch["paired"]):
                if not is_paired:
                    modality = batch["modality"][i]
                    break
            _, z = self.encode(img_t, modality)
