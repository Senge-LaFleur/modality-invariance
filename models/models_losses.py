...
CHANGELOG (fixes applied):
  1. mi_loss() replaced by mi_loss_vicreg() — adds variance + covariance terms
     to prevent representation collapse (VICReg-style, Bardes et al. 2022).
     The old mi_loss() is kept as mi_loss_legacy() for reference but should NOT
     be used in training.
  2. cross_modal_supcon_loss() added — computes supervised contrastive loss
     across z_c and z_d directly, so the loss actually learns cross-modal
     alignment rather than contrasting an already-fused embedding.
  3. skin_type_loss now accepts an optional lambda_skin weight (default 0.3)
     so it doesn't implicitly dominate the total loss.
  4. get_layer_wise_lr_params_vit() added as a proper exported helper
     (previously defined locally in each ViT train script — consolidated here).
"""
