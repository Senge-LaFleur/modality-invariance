...
All outputs saved in:
    - checkpoints_patchalign_vit/{clin,derm,both}/
    - results_patchalign_vit/{clin,derm,both}/
"""
...
# Argument parsing
# ============================================================
parser = argparse.ArgumentParser(description="PatchAlign ViT — modality ablation")
parser.add_argument(
    '--train_modality',
...
CFG = {
    ...
    'ckpt_dir':      WORK_ROOT / f'checkpoints_patchalign_vit' / TRAIN_MODALITY,
    'results_dir':   WORK_ROOT / f'results_patchalign_vit'     / TRAIN_MODALITY,
...
class PatchAlignViT(nn.Module):
    """
    Dual ViT-small encoder with PatchAlign MGOT alignment. Supports train_modality.
    """
    def __init__(
        self,
        embed_dim: int = 384,
        ...
        train_modality: str = 'both',
    ):
        super().__init__()
        self.train_modality = train_modality

        self.clinical_vit = timm.create_model(vit_model_name, pretrained=pretrained, num_classes=0)
        self.derm_vit     = timm.create_model(vit_model_name, pretrained=pretrained, num_classes=0)
    ...
    def forward(self, batch: dict) -> dict:
        ...
        if self.train_modality in ('clin', 'derm'):
            if self.train_modality == 'clin':
                ...
                z, patches, mask = self._extract_features(img_t, "derm")
            embeddings = z
            out_patches = patches
            out_masks = mask
        else:  # both regime
            if "clinical" in batch:
                ...
                z, patches, mask = self._extract_features(clinical_t, "clinical")
                embeddings = z
                out_patches = patches
                out_masks = mask
            else:
            ...
            # Enhance global embedding if paired
...
def main():
    ...
    plot_training_curves(history, f"Training History (PatchAlign ViT [{TRAIN_MODALITY}])",
    ...
    if 'clin' in test_loaders:
        ...
        plot_tsne(embs, labels_tsne,
                  f"t-SNE — Test [clin] (PatchAlign ViT, {TRAIN_MODALITY} trained)",
