...
CFG = {
    ...
    'batch_size':     32,
    'num_epochs':     10,       # Update as needed (e.g. 100)
    'lr':             1e-4,
    'min_lr':         1e-6,
    'weight_decay':   1e-4,
    'warmup_epochs':  3,       # Update as needed
...
def evaluate_test_loaders(model, test_loaders, device, cfg, results_dir, label_names, prefix="test"):
    ...
    Saves CSVs and plots for each split named  <prefix>_<modality>_*.
...
def main():
    ...
    # Load all data: train, val, test (clin/derm), paired_test (clin/derm), cross‑eval
    ...
    # ── Test on unpaired clin_test and derm_test ──────────────────────
    ...
    # ── Cross‑dataset evaluation (derm7pt clinical/derm) ────────────────
    ...
    for mod_name, metrics in cross_summary.items():
        print(f"  {mod_name}  acc={metrics['accuracy']:.4f}  "
              ...
              f"EOM={metrics['EOM']:.4f}  PQD={metrics['PQD']:.4f}")
    if cross_summary:
        # Save separately, one row per modality
