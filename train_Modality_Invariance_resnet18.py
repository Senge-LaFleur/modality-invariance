...
CHANGELOG vs previous version:
  1. Imports cross_modal_supcon_loss from models_losses.
  2. train_epoch: loss_con now uses cross_modal_supcon_loss(z_c, z_d, labels)
     instead of sup_con(z, labels) — this actually trains cross-modal alignment.
  3. train_epoch: loss_mi now uses the VICReg-based mi_loss() which prevents
     representation collapse.
  4. lambda_mi reduced from 0.8 → 0.15 to avoid MI loss overwhelming cls loss.
  5. lambda_skin added (0.3) so skin_type_loss has an explicit weight.
  6. warmup_epochs reduced from 100 → 50 (10% of 500 epochs).
  7. Paired-batch rate is logged each epoch so you can verify MI fires.
...
CFG = {
    ...
    'min_lr':         1e-6,
    'weight_decay':   1e-4,
    'warmup_epochs':  50,       # FIX: was 100; 50 ≈ 10% of 500 is more standard
...
def train_epoch(model, loader, optimizer, cfg, epoch, scaler, class_weights, device):
    ...
    for batch in pbar:
        ...
        with torch.cuda.amp.autocast(enabled=(device.type == "cuda")):
            ...
            # alignment.
            # Now: concatenate z_c and z_d and run SupCon across them.
            loss_con = 0.0
