#!/bin/bash
#SBATCH --job-name=train_BASE+SA+Contr_single
...
# ── Domain-shift modality argument ──────────────────────────────────────────
# This script trains ONE single-stream model on ONE modality. Submit it
# twice, once per modality, to get both directions of the domain-shift
# ablation:
#   sbatch "job_BASE+SA+Contr_single.sh" clinical
#   sbatch "job_BASE+SA+Contr_single.sh" derm
TRAIN_MODALITY="${1:-clinical}"
if [[ "$TRAIN_MODALITY" != "clinical" && "$TRAIN_MODALITY" != "derm" ]]; then
    echo "[ERROR] First argument must be 'clinical' or 'derm' (got '$TRAIN_MODALITY')."
    exit 1
fi
BACKBONE="vit_small"   # domain-shift ablation is ViT-only, per plan
...
OUTPUT_DIR="outputs_BASE+SA+Contr_single_${TRAIN_MODALITY}"

SCRIPT_NAME="../train_BASE+SA+Contr_single.py"
...
python "../$SCRIPT_NAME" "$WORK_DIR" "$OUTPUT_DIR" --train_modality "$TRAIN_MODALITY" --backbone "$BACKBONE"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo "Outputs (modality=$TRAIN_MODALITY, backbone=$BACKBONE):"
    echo "  Checkpoints → $WORK_ROOT/checkpoints_domain_shift_${TRAIN_MODALITY}_${BACKBONE}/"
    echo "  Results     → $WORK_ROOT/results_domain_shift_${TRAIN_MODALITY}_${BACKBONE}/"
