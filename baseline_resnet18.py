...
from evaluation import (
    ...
    plot_tsne,
    build_loaders,      # we reuse the same loader builder
    LABEL_NAMES,
)
...
CFG = {
    ...
    'ckpt_dir': WORK_ROOT / 'checkpoints_baseline_resnet18',
    'results_dir': WORK_ROOT / 'results_baseline_resnet18',
    ...
    'img_size': 224,
    'num_classes': 5,
    'num_skin_types': 6,        # not used in baseline
...
def main():
    ...
    # Model (use_projection=False for baseline)
    model = DualResNet18(
        embed_dim=CFG["embed_dim"],
        num_classes=CFG["num_classes"],
        num_skin_types=CFG["num_skin_types"],
        pretrained=True,
        use_projection=False,   # no projection head needed
    ).to(DEVICE)
